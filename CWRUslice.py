...
def get_files(root, test=False):
    ...
    data_root_1 = os.path.join(root, datasetname[0])
    data_root_2 = os.path.join(root, datasetname[3])
    path1 = os.path.join(data_root_2, normalname[0])
    data, lab = data_load(path1, axisname=normalname[0], label=0)

    for i in tqdm(range(len(dataname1))):
        path2 = os.path.join(data_root_1, dataname1[i])
...
class CWRUSlice(object):
    ...
    def data_preprare(self, test=False):
        ...
        if test:
            ...
            train_pd, val_pd = train_test_split_order(data_pd, test_size=0.6, num_classes= 10)
